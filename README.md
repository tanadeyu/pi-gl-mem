...
pi-mem（グローバル）と pi-local-mem（ローカル）は、**明示的に使い分ける**ことができます。AI に「グローバルに保存して」「ローカルに保存して」と伝えるだけで、それぞれ適切な場所に保存されます。ツール名はすべて `_local` 付きで混同しません：
...
**pi 起動時**にプロジェクトフォルダ直下に自動生成されます。初回起動時に `.pi-local-mem/` が存在しなければ自動作成され、2回目以降はそのデータを継続して使用します。
...
```
`pi uninstall` は `.pi-local-mem/` フォルダの既存データを削除しません。完全に消したい場合は別途 `rm -rf ./.pi-local-mem` を実行してください。

## 設定

`.pi-local-mem/pi_memory_local.json`（**プロジェクトごとに設定可能**）:
```json
...
※ @haha1903/pi-mem と併用する場合は pi-mem → pi-local-mem の順でインストールしてください
