### ?? プロジェクト専用メモリ拡張プラグイン「pi-local-mem」完全仕様書（最終決定版）
...
*   足元に `.pi-local-mem/` フォルダが存在しない場合、フォルダの新規作成、親切な初期解説付きの `MEMORY.md` 生成、および設定用 `pi_memory.json` の雛形を一瞬で自動生成する。
...
*   起動時に `.pi-local-mem/pi_memory.json` を読み込む。ファイルが存在しない場合や、ユーザーがカンマを忘れるなどの記述エラー（構文エラー）を起こした場合でも、システムを強制終了させず、自動的にデフォルト（`injectLocal: true`）にフォールバックして安全に起動する。
...
### ?? 4. 自己完結型プラグイン全ソースコード（Node.js / 約65行）
...
javascript
     ...
     * 【2. 使い方】
     * インストール後、通常通り「pi」を起動。
     * 起動した瞬間に、足元に「.pi-local-mem/本地」が生成され、本ファイル自身を含むすべてが隠蔽されます。
     * 
     * 【3. 安全な融合作業（注意・免責事項）】
